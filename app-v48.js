...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=46').then(()=>updatePwaStatus()).catch(()=>updatePwaStatus()));
}else{window.addEventListener('load',updatePwaStatus);}

scheduleReminderChecks();
render();
setTimeout(updatePwaStatus,200);
// V45: scratch pad moved to Studio; old emergency calendar mount removed.
   return;
 }
 let id=uid();
 let number=nextSupplyNumber();
