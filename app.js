...
function ensureDay(k){
  if(!state.calendarData[k]) state.calendarData[k]={agenda:[],tasks:[],notes:''};
  if(!state.calendarData[k].agenda) state.calendarData[k].agenda=[];
  if(!state.calendarData[k].tasks) state.calendarData[k].tasks=[];
  if(state.calendarData[k].notes===undefined) state.calendarData[k].notes='';
  return state.calendarData[k];
}
...
function render(){
 ...
 subtabs.innerHTML=SECTIONS[state.section].tabs.map(t=>`
   <button class="subtab ${state.tabs[state.section]===t[0]?'active':''}" onclick="setTab('${t[0]}')">
     ${t[1]}
   </button>
 `).join('');
...
function pageTemplate(section,tab){
 ...
 <div class="titleRow">
   ...
   <div class="note">Blank Page Template</div>
 </div>

 <div class="layout">
  <div class="box">
    <h3>Main Content Area</h3>
    <p>This is where the ${tab[1]} tools will go.</p>
    <div class="line"></div>
    <div class="line"></div>
    <div class="line"></div>
  ...
  <div class="box">
    <h3>Side / Detail Area</h3>
    <p>Selected records, trackers, or linked info will go here.</p>
    <textarea style="width:100%;min-height:120px;border-radius:10px;padding:8px"
      oninput="state.notes['${key}']=this.value;save()">${state.notes[key]||''}</textarea>
  </div>
 </div>`;
}
...
function renderScheduleCalendar(){
 ...
 let monthTabs=MONTHS.map((name,i)=>`
   <button class="monthBtn ${i===m?'active':''}" onclick="setCalendarMonth(${i})">${name}</button>
 `).join('');
 ...
 for(let i=0;i<first;i++){
   grid+=`<div class="calDay blank"></div>`;
 }
 ...
 for(let d=1; d<=total; d++){
   ...
   let previews=agenda.slice(0,3).map(item=>`
     <div class="preview ${item.canceled?'canceled':''}">
       ${formatTime(item.time)}: ${escapeHtml(item.title||'')}
     </div>
   `).join('');

   if(agenda.length>3){
     previews+=`<div class="preview">+${agenda.length-3} more</div>`;
   }

   grid+=`
   <div class="calDay ${k===state.selectedDate?'selected':''} ${k===todayKey?'today':''}"
        onclick="selectCalendarDay('${k}')">
 ...
 return `
 <div class="monthTabs">${monthTabs}</div>

 <div class="calendarTitle">
   <button onclick="changeCalendarYear(-1)">‹ ${y-1}</button>
   <h2>${FULL_MONTHS[m]} ${y}</h2>
   <button onclick="changeCalendarYear(1)">${y+1} ›</button>
 </div>
 ...
 <div class="dayBottom">
   <div class="dailyBox">
     ...
     <div>
       ${selectedData.agenda.map((item,idx)=>`
         <div class="agendaRow ${item.canceled?'canceled':''}">
           <span>${formatTime(item.time)}: ${escapeHtml(item.title||'')}</span>
           <button class="smallBtn" onclick="toggleCancel(${idx})">×</button>
         </div>
       `).join('')||'<p class="note">No agenda items yet.</p>'}
     </div>

     <div class="addRow">
       <input id="newAgendaTime" placeholder="8a">
       <input id="newAgendaTitle" placeholder="Weed Eating">
     ...
     <div>
       ${selectedData.tasks.map((task,idx)=>`
         <div class="taskRow ${task.done?'done':''}">
           <input type="checkbox" ${task.done?'checked':''} onchange="updateTask(${idx},'done',this.checked)">
           <input type="text" value="${escapeHtml(task.text||'')}" oninput="updateTask(${idx},'text',this.value)">
           <button class="smallBtn" onclick="deleteTask(${idx})">×</button>
         </div>
...
function renderJobForm(){
 ...
 <div class="titleRow">
   <div>
     <h2>New Job Entry</h2>
     <p>Add scheduled or completed jobs. Saved jobs show on the calendar.</p>
   </div>
   <div class="note">Schedule / Job</div>
 </div>

 <div class="box">
   <label>Date</label>
   <input id="jobDate" type="date" value="${state.selectedDate}">
   ...
   <input id="jobEnd" placeholder="optional">

   <label>Client Name</label>
   <input id="jobClient" placeholder="Client name">
   ...
   <label>Hours</label>
   <input id="jobHours" type="number" step="0.25" placeholder="Hours">

   <label>Amount Owed</label>
   <input id="jobOwed" type="number" step="0.01" placeholder="0.00">

   <label>Amount Received</label>
   <input id="jobReceived" type="number" step="0.01" placeholder="0.00">

   <label>Notes</label>
   <textarea id="jobNotes" placeholder="Job notes..."></textarea>

   <div class="actions">
     <button class="save" onclick="saveJob()">Save Job</button>
     <button onclick="setTab('calendar')">Back</button>
   </div>
 </div>`;
}
...
function renderEventForm(){
 ...
 <div class="titleRow">
   <div>
     ...
     <p>Add personal events. Events show on the calendar but do not count as jobs.</p>
   </div>
   <div class="note">Schedule / Event</div>
 </div>

 <div class="box">
   <label>Date</label>
   <input id="eventDate" type="date" value="${state.selectedDate}">

   <label>Title</label>
   <input id="eventTitle" placeholder="Appointment">

   <label>Time</label>
   <input id="eventTime" placeholder="2p">
   ...
   <div class="actions">
     <button class="save" onclick="saveEvent()">Save Event</button>
     <button onclick="setTab('calendar')">Back</button>
   </div>
 </div>`;
}
...
function setCalendarMonth(m){
 state.month=m;
 state.selectedDate=makeKey(state.year,m,1);
 save();
 render();
}

function changeCalendarYear(n){
 state.year+=n;
 state.selectedDate=makeKey(state.year,state.month,1);
 save();
 render();
}
...
function saveJob(){
 ...
 d.agenda.push({
   ...
   owed:document.getElementById('jobOwed').value,
   received:document.getElementById('jobReceived').value,
...
function toggleCancel(idx){
 let d=ensureDay(state.selectedDate);
 d.agenda[idx].canceled=!d.agenda[idx].canceled;
 save();
 render();
}

function updateTask(idx,key,value){
 ensureDay(state.selectedDate).tasks[idx][key]=value;
 save();
 renderCalendarOnlySoon();
}

function deleteTask(idx){
 ensureDay(state.selectedDate).tasks.splice(idx,1);
 save();
 render();
}
...
function addTask(){
 ...
 ensureDay(state.selectedDate).tasks.push({
   text,
   done:false
 });
...
function updateNotes(value){
 ensureDay(state.selectedDate).notes=value;
 save();
}

function sortAgenda(arr){
 arr.sort((a,b)=>timeValue(a.time)-timeValue(b.time));
}
...
function renderCalendarOnlySoon(){
 clearTimeout(softRenderTimer);
 softRenderTimer=setTimeout(()=>render(),650);
}
...
function backup(){
 ...
 a.href=u;
 a.download='bird-planner-backup.json';
 a.click();
