...
const now=new Date();
let state=JSON.parse(localStorage.getItem(KEY)||'null')||{section:'schedule',tabs:{schedule:'calendar',clients:'directory',supplies:'list',banking:'accounts'},notes:{},year:now.getFullYear(),month:now.getMonth(),selectedDate:dateKey(now),calendarData:{},clients:{},selectedClient:'',invoices:[],invoiceCounter:1,services:['Weed Eating','Mowing','Painting','Leaf Blowing','Yard Clean','General Labor','Debris Removal','Gutters','Handyman','Other'],supplies:['Gas','Oil','String','Other'],supplyItems:{},selectedSupplyId:'',supplyReceipts:[],supplyCounter:1};
function ensureCollections(){if(!state.clients)state.clients={};if(!state.invoices)state.invoices=[];if(!state.invoiceCounter)state.invoiceCounter=1;if(!state.services)state.services=['Weed Eating','Mowing','Painting','Leaf Blowing','Yard Clean','General Labor','Debris Removal','Gutters','Handyman','Other'];if(!state.supplies)state.supplies=['Gas','Oil','String','Other'];if(!state.supplyItems)state.supplyItems={};if(!state.supplyReceipts)state.supplyReceipts=[];if(!state.supplyCounter)state.supplyCounter=1;if(!state.tabs)state.tabs={schedule:'calendar',clients:'directory',supplies:'list',banking:'accounts'};if(!state.tabs.supplies)state.tabs.supplies='list'}
...
function renderScheduleCalendar(){let y=state.year||now.getFullYear(),m=state.month??now.getMonth();state.year=y;state.month=m;let first=new Date(y,m,1).getDay(),total=new Date(y,m+1,0).getDate(),todayKey=dateKey(new Date());let monthTabs=MONTHS.map((name,i)=>`<button class="monthBtn ${i===m?'active':''}" onclick="setCalendarMonth(${i})">${name}</button>`).join('');let days=['SUN','MON','TUE','WED','THU','FRI','SAT'].map(d=>`<div>${d}</div>`).join('');let grid='';for(let i=0;i<first;i++)grid+=`<div class="calDay blank"></div>`;for(let d=1;d<=total;d++){let k=makeKey(y,m,d),data=ensureDay(k),agenda=data.agenda||[];let previews=agenda.slice(0,3).map(item=>`<div class="preview ${item.canceled?'canceled':''}">${formatTime(item.time)}: ${escapeHtml(item.title||'')}</div>`).join('');if(agenda.length>3)previews+=`<div class="preview">+${agenda.length-3} more</div>`;grid+=`<div class="calDay ${k===state.selectedDate?'selected':''} ${k===todayKey?'today':''}" onclick="selectCalendarDay('${k}')"><div class="dayNum">${d}</div>${previews}</div>`}let selectedData=ensureDay(state.selectedDate),selectedDateObj=new Date(state.selectedDate+'T12:00:00');return `<div class="monthTabs">${monthTabs}</div><div class="calendarTitle"><button onclick="changeCalendarYear(-1)">‹ ${y-1}</button><h2>${FULL_MONTHS[m]} ${y}</h2><button onclick="changeCalendarYear(1)">${y+1} ›</button></div><div class="weekdays">${days}</div><div class="calendarGrid">${grid}</div><div class="dayBottom"><div class="dailyBox"><h3>${selectedDateObj.toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric',year:'numeric'})}</h3><h4>Agenda</h4><div>${selectedData.agenda.map((item,idx)=>`<div class="agendaRow ${item.canceled?'canceled':''}"><span>${formatTime(item.time)}: ${escapeHtml(item.title||'')}</span><button class="smallBtn" onclick="toggleCancel(${idx})">×</button></div>`).join('')||'<p class="note">No agenda items yet.</p>'}</div><div class="addRow"><input id="newAgendaTime"><input id="newAgendaTitle"><button onclick="addAgenda()">Add</button></div></div><div class="dailyBox"><h4>Tasks</h4><div>${selectedData.tasks.map((task,idx)=>`<div class="taskRow ${task.done?'done':''}"><input type="checkbox" ${task.done?'checked':''} onchange="updateTask(${idx},'done',this.checked)"><input type="text" value="${escapeHtml(task.text||'')}" oninput="updateTask(${idx},'text',this.value)"><button class="smallBtn" onclick="deleteTask(${idx})">×</button></div>`).join('')||'<p class="note">No tasks yet.</p>'}</div><div class="addTaskRow"><input id="newTaskText"><button onclick="addTask()">Add</button></div><h4>Notes</h4><textarea class="notesArea" oninput="updateNotes(this.value)">${escapeHtml(selectedData.notes||'')}</textarea></div></div>`}
function renderJobForm(){let names=Object.keys(state.clients||{}).sort();return `<div class="titleRow"><div><h2>New Job Entry</h2><p>Saves to calendar, creates client file, and starts/updates client invoice.</p></div><div class="note">Schedule / Job</div></div><div class="box"><datalist id="clientNames">${names.map(n=>`<option value="${escapeHtml(n)}"></option>`).join('')}</datalist><label>Date</label><input id="jobDate" type="date" value="${state.selectedDate}"><label>Job Status</label><select id="jobStatus"><option value="scheduled">Scheduled Job</option><option value="completed">Completed Job</option></select><label>Title / Service</label><input id="jobTitle"><div class="two"><div><label>Time In</label><input id="jobStart" oninput="calcJobPay()"></div><div><label>Time Out</label><input id="jobEnd" oninput="calcJobPay()"></div></div><label>Total Hours</label><input id="jobHours" type="number" step="0.25" oninput="calcJobPay()"><div class="two"><div><label>Pay Type</label><select id="jobPayType" onchange="calcJobPay()"><option value="hourly">Hourly</option><option value="flat">Flat Rate</option></select></div><div><label>Rate / Flat Amount</label><input id="jobRate" type="number" step="0.01" oninput="calcJobPay()"></div></div><label>Total Price</label><input id="jobOwed" type="number" step="0.01" readonly><label>Amount Received</label><input id="jobReceived" type="number" step="0.01"><label>Client Name</label><input id="jobClient" list="clientNames" oninput="autoFillClient()"><label>Phone</label><input id="jobPhone"><label>Address</label><input id="jobAddress"><label>Notes</label><textarea id="jobNotes"></textarea><div class="actions"><button class="save" onclick="saveJob()">Save Job</button><button onclick="setTab('calendar')">Back</button></div></div>`}
function renderEventForm(){return `<div class="titleRow"><div><h2>New Event Entry</h2><p>Add personal events. Events show on the calendar but do not count as jobs.</p></div><div class="note">Schedule / Event</div></div><div class="box"><label>Date</label><input id="eventDate" type="date" value="${state.selectedDate}"><label>Title</label><input id="eventTitle"><label>Time</label><input id="eventTime"><label>Location</label><input id="eventLocation"><label>Notes</label><textarea id="eventNotes"></textarea><div class="actions"><button class="save" onclick="saveEvent()">Save Event</button><button onclick="setTab('calendar')">Back</button></div></div>`}
...
function renderSupplyList(){
 ...
 return `<div class="titleRow"><div><h2>Supplies List</h2><p>Quick add a basic item on the first line, or tap Add Item to open the full blank item form.</p></div><button class="save" type="button" onclick="newSupplyItem()">+ Add Item</button></div><div class="box"><div class="supplyHeader"><b>Item #</b><b>Item Name</b><b>Qty Remaining</b><b>Action</b></div><div class="supplyRowList quickAddSupply" title="Quick add a supply item"><span class="quickPlus">+</span><input id="quickSupplyName" placeholder="Item name" onkeydown="quickSupplyKey(event)"><input id="quickSupplyPrice" type="number" step="0.01" placeholder="Price" onkeydown="quickSupplyKey(event)"><button class="save smallBtn" type="button" onclick="addQuickSupplyFromList()">Add</button></div>${items.map(item=>`<div class="supplyRowList"><span onclick="openSupplyItem('${item.id}')">${item.itemNumber}</span><span onclick="openSupplyItem('${item.id}')">${escapeHtml(item.name||'Untitled Supply')}</span><span onclick="openSupplyItem('${item.id}')">${formatQty(item.quantityRemaining,item.unit)}</span><button class="delete smallBtn" type="button" onclick="deleteSupplyItemFromList(event,'${item.id}')">Delete</button></div>`).join('')||'<p class="note">No supplies yet. Use the quick-add row above or tap + Add Item.</p>'}</div>`
}
...
function newSupplyItem(){ensureCollections();let id=uid();let number=nextSupplyNumber();let item={id,itemNumber:number,name:'',description:'',storeItemNumber:'',price:'',quantityForPrice:'',unit:'unit',pricePerUnit:'',quantityRemaining:0,supplier:'',photo:'',inventoryLog:[],isDraft:true};state.supplyItems[id]=item;state.selectedSupplyId=id;state.section='supplies';state.tabs.supplies='item';save();render()}
...
function recalcSupplyRemaining(id){
 let item=state.supplyItems?.[id];
 if(!item)return;
 ...
 if(el)el.value=item.quantityRemaining;
 save();
}
...
function invoiceEditorHtml(inv){let serviceOptions=(state.services||[]).map(s=>`<option value="${escapeHtml(s)}"></option>`).join('');let supplyOptions=(state.supplies||[]).map(s=>`<option value="${escapeHtml(s)}"></option>`).join('');return `<div class="titleRow"><div><h2>Invoice #${inv.number}</h2><p>${escapeHtml(inv.client||'No client selected')} • ${getInvoiceStatus(inv)}</p></div><button onclick="setTab('invoices')">Back</button></div><div class="invoiceGrid"><div class="box"><label>Client</label><input id="invClient" value="${escapeHtml(inv.client||'')}" oninput="updateInvoiceField('${inv.id}','client',this.value)"><label>Date</label><input id="invDate" value="${escapeHtml(inv.date||'')}" oninput="updateInvoiceField('${inv.id}','date',this.value)"><h3>Services</h3><datalist id="serviceOptions">${serviceOptions}</datalist><div>${(inv.services||[]).map((s,idx)=>`<div class="lineItem"><span>${escapeHtml(s.name)}${s.qty?` (${s.qty})`:''}</span><input type="number" step="0.01" value="${Number(s.amount||0)}" oninput="updateInvoiceLine('${inv.id}','services',${idx},this.value)"><button class="smallBtn" onclick="removeInvoiceLine('${inv.id}','services',${idx})">×</button></div>`).join('')||'<p class="note">No services yet.</p>'}</div><div class="two"><input id="newServiceName" list="serviceOptions"><input id="newServiceAmount" type="number" step="0.01"></div><button onclick="addInvoiceService('${inv.id}')">Add Service</button><h3>Supplies</h3><datalist id="supplyOptions">${supplyOptions}</datalist><div>${(inv.supplies||[]).map((s,idx)=>`<div class="lineItem"><span>${escapeHtml(s.name)}${s.qty?` (${s.qty})`:''}</span><input type="number" step="0.01" value="${Number(s.amount||0)}" oninput="updateInvoiceLine('${inv.id}','supplies',${idx},this.value)"><button class="smallBtn" onclick="removeInvoiceLine('${inv.id}','supplies',${idx})">×</button></div>`).join('')||'<p class="note">No supplies yet.</p>'}</div><div class="two"><input id="newSupplyName" list="supplyOptions"><input id="newSupplyQty" type="number" step="0.01"></div><button onclick="addInvoiceSupply('${inv.id}')">Add Supply</button><h3>Payment</h3><label>Amount Paid</label><input id="invPaid" type="number" step="0.01" value="${Number(inv.paid||0)}" oninput="updateInvoicePaid('${inv.id}',this.value)"><label>Notes</label><textarea id="invNotes" oninput="updateInvoiceField('${inv.id}','notes',this.value)">${escapeHtml(inv.notes||'')}</textarea><h3>Signature</h3><canvas id="signaturePad" class="signature"></canvas><div class="actions"><button onclick="clearInvoiceSignature('${inv.id}')">Clear Signature</button></div><h3>Check Photo</h3><input type="file" accept="image/*" capture="environment" onchange="attachCheckPhoto(event,'${inv.id}')">${inv.checkPhoto?`<img class="photo" src="${inv.checkPhoto}">`:''}<div class="actions"><button class="save" onclick="shareInvoice('${inv.id}','text')">Text Invoice</button><button class="save" onclick="shareInvoice('${inv.id}','email')">Email Invoice</button><button onclick="shareInvoice('${inv.id}','share')">Share</button><button class="delete" onclick="deleteInvoice('${inv.id}')">Delete</button></div></div><div class="box receipt">${invoiceReceiptHtml(inv)}</div></div>`}
function invoiceReceiptHtml(inv){return `<h2>Invoice #${inv.number}</h2><p><b>Client:</b> ${escapeHtml(inv.client||'')}<br><b>Date:</b> ${escapeHtml(inv.date||'')}</p><h3>Services</h3>${(inv.services||[]).map(s=>`<div class="receiptLine"><span>${escapeHtml(s.name)}</span><b>${money(s.amount)}</b></div>`).join('')||'<p>No services.</p>'}<h3>Supplies</h3>${(inv.supplies||[]).map(s=>`<div class="receiptLine"><span>${escapeHtml(s.name)}</span><b>${money(s.amount)}</b></div>`).join('')||'<p>No supplies.</p>'}<hr><p><b>Total:</b> ${money(inv.total)}<br><b>Paid:</b> ${money(inv.paid)}<br><b>Balance:</b> ${money(invoiceBalance(inv))}<br><b>Status:</b> ${getInvoiceStatus(inv)}</p>${inv.signature?`<p><b>Signed:</b><br><img class="sigImg" src="${inv.signature}"></p>`:''}${inv.checkPhoto?'<p><b>Check photo saved.</b></p>':''}`}
function recalcInvoice(inv){inv.total=[...(inv.services||[]),...(inv.supplies||[])].reduce((a,l)=>a+Number(l.amount||0),0);inv.status=getInvoiceStatus(inv).toLowerCase();save()}function updateInvoiceField(id,key,value){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;inv[key]=value;save()}function updateInvoicePaid(id,value){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;inv.paid=Number(value||0);recalcInvoice(inv);renderInvoiceEditor(id)}function updateInvoiceLine(id,type,idx,value){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;inv[type][idx].amount=Number(value||0);recalcInvoice(inv)}function addInvoiceService(id){let inv=state.invoices.find(i=>i.id===id);let name=document.getElementById('newServiceName').value.trim(),amount=Number(document.getElementById('newServiceAmount').value||0);if(!inv||!name)return;addServiceToDb(name);inv.services.push({name,amount});recalcInvoice(inv);renderInvoiceEditor(id)}function addInvoiceSupply(id){let inv=state.invoices.find(i=>i.id===id);let name=document.getElementById('newSupplyName')?.value.trim(),qty=Number(document.getElementById('newSupplyQty')?.value||0);if(!inv||!name)return;if(!qty||qty<0)qty=1;let item=findSupplyByName(name);let amount=item?Number(item.pricePerUnit||0)*qty:0;addSupplyToDb(name);inv.supplies.push({name,amount,qty,supplyId:item?item.id:''});recalcInvoice(inv);if(item)recalcSupplyRemaining(item.id);save();renderInvoiceEditor(id)}function removeInvoiceLine(id,type,idx){let inv=state.invoices.find(i=>i.id===id);if(!inv)return;let line=inv[type][idx];inv[type].splice(idx,1);recalcInvoice(inv);if(type==='supplies'&&line?.supplyId)recalcSupplyRemaining(line.supplyId);renderInvoiceEditor(id)}
